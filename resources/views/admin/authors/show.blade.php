...
@prepend('breadcrumbs')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item"><a href="/admin/authors">Penulis</a></li>
                ...
                <div class="card card-primary">
                    <div class="card-body">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-2 text-center mb-3 mb-md-0">
                                        <img src="https://via.placeholder.com/150x100.png/{{ substr(uniqid(), -3) }}?text={{ urlencode(strtok($author->name, ' ')) }}"
                                            alt="{{ $author->name }}" class="img-fluid"
                                            style="height:100px; object-fit: cover">
                                    </div>
                                    <div class="col-md-4 pt-md-3">
                                        <dl class="row">
                                            <dt class="col-sm-4">Nama Penulis</dt>
                                            <dd class="col-sm-8">{{ $author->name }}</dd>
                                            <dt class="col-sm-4">Total Buku</dt>
                                            <dd class="col-sm-8">{{ $author->books->count() }}</dd>
                                        </dl>
                                    </div>
                                    <div class="col-md-4 pt-md-3">
                                        <dl class="row">
                                            ...
                                            <dd class="col-sm-8">
                                                {{ date('d-m-Y, H:i', strtotime($author->created_at)) }}</dd>
                                            <dt class="col-sm-4">Updated at</dt>
                                            <dd class="col-sm-8">
                                                {{ date('d-m-Y, H:i', strtotime($author->updated_at)) }}</dd>
                                        </dl>
                                    </div>
                                </div>
                            </div>
                        ...
                        @if (count($author->books) != 0)
                            <h4 class="mt-4 mb-3">Semua buku dalam Penulis {{ $author->name }}</h4>
                                            ...
                                            <small> in :
                                                @foreach ($book->book_authors as $author)
                                                    <a href="/authors/{{ $author->author->id }}"
                                                        class="text-decoration-none text-primary">{{ $author->author->name }}</a>@if (!$loop->last), @endif
                                                @endforeach
                                            </small>
                                        </div>
                                    </div>
