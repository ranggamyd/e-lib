                        ...
                        <div class="row">
                            <div class="col-12 col-md-2 text-center mb-3 mb-md-0">
                                <a href="/admin/categories/{{ $category->id }}">
                                    <img src="https://via.placeholder.com/75x75.png/{{ substr(uniqid(), -3) }}?text={{ urlencode(strtok($category->name, ' ')) }}"
                                        class="img-fluid img-thumbnail rounded" alt="{{ $category->name }}"
                                        style="height: 75px; object-fit: cover">
                                </a>
                            </div>
                            <div class="col d-flex align-items-center">
                                <a href="/admin/categories/{{ $category->id }}" class="text-decoration-none">
                                    <h5 class="mb-0">{{ $category->name }}</h5>
                                    <p class="text-muted mt-2">Total {{ $category->books->count() }} Buku</p>
                                </a>
                            </div>
                        </div>
                    </div>
